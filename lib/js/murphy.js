    ...
    $.each(data.data, function (index, val) {
        ...
        $.each(val.others, function (index1, val1) {
            ...
            var partRow = $('<span>').attr("style",
                "font-size:100%;cursor:pointer;").text(val1.name);
            td1.append(partRow);
            td2 = $('<td>').attr("style", "width:50px").attr("align", "left");
            tr.append(td2);
            var playPartButton = Murphy.createPlayButton();
            td2.append(playPartButton);
            var source = new SoundSource(data.phrase_path + "" + val1.source, 4,
                jumpID, key, partRow);
            MURPHY.sources.push(source);
            unitSources.push(source);
...
function SoundManager() {
    ...
    this.play = function (el, label, delay, speed, ignoreConfig) {
        ...
        if (!isNullAndUndef(this.engTextElement)) {
            $(this.engTextElement).css("color", "black");
            this.engTextElement = null;
        }
    ...
    var loopPlay = function (soundManager) {
        ...
        if (!isNullAndUndef(soundManager.engTextElement)) {
            $(soundManager.engTextElement).css("color", "black");
            soundManager.engTextElement = null;
        }
        if (!isNullAndUndef(sound_source.engTextElement)) {
            $(sound_source.engTextElement).css("color", "blue");
            soundManager.engTextElement = sound_source.engTextElement;
        }
...
function SoundSource(srcPath, label, jumpID, unit, engTextElement) {
    this.src = srcPath;
    this.label = label;
    this.jumpID = jumpID;
    this.unit = unit;
    this.engTextElement = engTextElement;
}
